...
const Contact = () => {
	const tecnologies = [
		{
			text: '<u>+55 (21) 98284-9948</u>',
			icon: WhatsappIcon,
			link: 'https://api.whatsapp.com/send?phone=5521982849948',
		},
		{ text: '<u>contato@lmvadvogados.adv.br</u>', icon: EmailIcon, link: '' },
		{
			text: '<u>Agende agora mesmo<br /> uma reunião no ZOOM</u>',
			icon: CameraIcon,
			link: '',
		},
		{
			text: 'Empresarial Cândido Mendes, RJ<br /> Centro Empresarial Varig, DF',
			icon: LocationIcon,
			link: '',
		},
	];
			...
			<div className="w-full grid lg:grid-cols-2 gap-[26px] lg:gap-24">
				<div className="w-full flex flex-col gap-[24px] lg:max-w-[600px]">
					...
					<span className="text-[1.4rem] lg:text-[1.8rem] font-normal w-full lg:max-w-[680px] tracking-[-0.36px] leading-[160%] mb-[40px]">
						...
						videochamada por Zoom. Você pode contar com uma resposta rápida,
						tratamento respeitoso e uma preocupação focada em sua questão
						jurídica.
					</span>
				</div>
				...
				<div className="grid md:grid-cols-2 w-full lg:max-w-[640px] gap-4">
					{tecnologies.map((tecnology, idx) => {
						return (
							<a key={idx} href={tecnology.link}>
								<div
									key={tecnology.text}
									className="px-[24px] flex flex-col items-center gap-12 h-auto py-[40px] text-center bg-[#FAFAFA]">
									<Image src={tecnology.icon} alt="" />

									<span
										className="text-[1.4rem] w-[102%] lg:text-[1.8rem] leading-[160%] tracking-[-0.36px]"
